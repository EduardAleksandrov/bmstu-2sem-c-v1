// работа со связанным списком, + локализация языка + сортировка - не работает
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
...
void sortList(struct clients **head)
{
    puts("введите 1 для сортировки по id");
    puts("введите 2 для сортировки по возрасту");
    int sortNumber;
    scanf(" %d", &sortNumber);

    struct clients *current = *head;
    unsigned long int sumAllStructers = 0;
    if(*head == NULL)
    {
        puts("Нет данных");
        return;
    ...
    if(sortNumber == 1)
    {
        struct clients *listVar;
        for(unsigned long int i = 0; i < sumAllStructers-1; i++)
        {
            listVar = current;
            for(unsigned long int j = i + 1; j < sumAllStructers; j++)
            {
                listVar = listVar->next;
                if(current->id > listVar->id)
                {
